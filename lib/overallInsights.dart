...
class _InsightsPageState extends State<InsightsPage> {
  ...
  Future<void> _fetchData(String duration) async {
    setState(() => _isLoading = true); // ✅ Show loader before fetching data
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _selectedDuration = duration;
          _totalSpent = (data['total_spent'] ?? 0).toDouble();
          _categories = List<Map<String, dynamic>>.from(data['categories']);
        });
      } else {
        print('Failed to fetch data: ${response.statusCode}');
      ...
      setState(() => _isLoading = false); // ✅ Hide loader after data fetch
    }
  }
