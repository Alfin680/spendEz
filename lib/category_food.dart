...
// Modify _loadBudget to use it
  ...
  Future<void> _fetchSpendingData() async {
    final url = Uri.parse(
        "http://10.0.2.2:5000/category-spending-week?user_id=${widget.userId}&category=Food"); // Replace "Food" with the desired category
    try {
  ...
  Future<void> _fetchBudgetSpent() async {
    final url = Uri.parse(
        "http://10.0.2.2:5000/category-budget-spent?user_id=${widget.userId}&category=Food"); // Replace "Food" with the desired category
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
